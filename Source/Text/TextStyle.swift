...
public class TextStyle: NSObject {
  ...
  public func trueTextBoundsAndViewFrame(for textType: TargetTextType, boundBy boundByIn: CGSize, usePreciseTextAlignments: Bool, containerSize: CGSize = .zero) -> (CGRect, CGRect) {
    ...
    if usePreciseTextAlignments {
      ...
      size = CGSize(width: rect.width.halfPointCeil, height: rect.height.halfPointCeil)
    }
    ...
    size = CGSize(width: width.halfPointCeil, height: height.halfPointCeil)
    ...
    containerFrame.size.height = containerFrame.height.halfPointAligned
    ...
    let heightDiff = (containerSize.height - size.height).halfPointAligned
...
public extension TextStyle {
  func applying(textAlpha: CGFloat) -> TextStyle {
    guard let descriptor = textDescriptors.first else { return self }
    let textColor = descriptor.textAttributes.textColor.color(withAlpha: textAlpha)
    descriptor.textAttributes.textColor = textColor
    return TextStyle(textDescriptors: [descriptor])
  }
}
